...
export default function ParentPortal({ profile, onLogout, theme, setTheme }) {
  ...
  if (!isMobile) {
    const NAV = [
      ...
      { id: "practice", label: "Casa",    icon: "trend",    badge: 0 },
    ...
    return (
      <>
          ...
          <main id="main" style={{ padding: "28px 40px 60px", maxWidth: 1000, width: "100%", margin: "0 auto" }}>
            <div style={{ marginBottom: 22 }}>
              ...
              <p style={{ fontSize: 14.5, color: "#8A8A86", marginTop: 6 }}>
                {myChildren.length === 0 ? "Nenhum filho associado." : `${myChildren.length} ${myChildren.length === 1 ? "filho acompanhado" : "filhos acompanhados"} pela Casa.`}
              </p>
            </div>
        ...
        {/* Modal pedido troca */}
        {requestOpen && (
          <div onClick={() => setRequestOpen(false)} className="modal-overlay" role="dialog" aria-modal="true" style={{ position: "fixed", inset: 0, background: "rgba(21,39,65,.4)", backdropFilter: "blur(4px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div onClick={(e) => e.stopPropagation()} className="modal-panel" style={{ background: "#FFFFFF", borderRadius: 18, width: "100%", maxWidth: 460, padding: 22 }}>
              ...
              <div className="serif" style={{ fontSize: 22, fontWeight: 300, color: "#152741", marginTop: 4, marginBottom: 14 }}>Troca de horário</div>
              <Field label="Filho"><Sel value={requestForm.patient_id} onChange={(v) => setRequestForm((f) => ({ ...f, patient_id: v }))} options={myChildren.map((c) => ({ v: c.id, l: c.name }))} placeholder="Selecionar..." /></Field>
              <Field label="Novo dia"><Sel value={requestForm.new_day} onChange={(v) => setRequestForm((f) => ({ ...f, new_day: v }))} options={DAYS.map((d) => ({ v: d, l: d }))} placeholder="Selecionar..." /></Field>
              <Field label="Nova hora"><Sel value={requestForm.new_hour} onChange={(v) => setRequestForm((f) => ({ ...f, new_hour: v }))} options={HOURS.map((h) => ({ v: h, l: h }))} placeholder="Selecionar..." /></Field>
              <Field label="Motivo (opcional)"><Inp value={requestForm.reason} onChange={(e) => setRequestForm((f) => ({ ...f, reason: e.target.value }))} placeholder="Ex: nova rotina" /></Field>
